...
export class AiAssistantWidget extends Component {
	...
	async sendMessage() {
		...
		// 1. Basic validation: Check if empty, loading, or not configured
		...
		// 2. Prohibited Content Filter (Local Guard)
		// This implements the limits to prevent abuse and distraction [cite: 4, 9]
		const prohibitedKeywords = [
			'pornography', 'prostitution', 'drugs', 'terrorism', 
			'extreme violence', 'hacking', 'tax evasion', 'suicide'
		];
		
		const lowerQuestion = question.toLowerCase();
		if (prohibitedKeywords.some(keyword => lowerQuestion.includes(keyword))) {
			this.state.messages.push({
				text: "⚠️ I cannot assist with this request. My main role is to help you work more efficiently in the CRM.",
				sender: "system",
				timestamp: new Date(),
				isError: true,
			});
			...
			return; // Stop the execution here [cite: 10]
		}
	...
	async copyToClipboard(text) {
	    try {
	        ...
	        this.notificationService.add("📋 Copied to clipboard", { type: "info" });
	    } catch (err) {
	        this.notificationService.add("Failed to copy", { type: "warning" });
	    }
	}
