...
export class Dashboard extends Component {
	...
	buildQuickStats(kpiGroups) {
		...
		const statBlueprints = [
			{
				key: "total_leads",
				label: _t("Total Leads"),
				note: _t("Leads in selected range"),
				accent: "leads",
			...
			},
			{
				...
				note: _t("Revenue in selected range"),
				accent: "sales",
				metric: pickMetric("range_invoicing", "monthly_invoicing", "total_income"),
			},
			{
				key: "total_invoices",
				label: _t("Total Invoices"),
				note: _t("Issued invoice volume"),
				accent: "invoices",
	...
	buildPdfHtml() {
			...
			.map(
				(group) => `
					<section class="pdf_section">
						...
						<table>
							<thead>
								<tr><th>${_t("Metric")}</th><th>${_t("Value")}</th></tr>
							</thead>
							<tbody>
								${(group.metrics || [])
						.map(
		...
		const topClientsSection = topClientsRows
			? `
				<section class="pdf_section">
					<h2>${_t("Top Clients")}</h2>
					<table>
						<thead>
							<tr><th>#</th><th>${_t("Client")}</th><th>${_t("Revenue")}</th></tr>
						</thead>
		...
		const alertsSection = alertsRows
			? `
				<section class="pdf_section">
					...
					<table>
						<thead>
							<tr><th>${_t("Alert")}</th><th>${_t("Value")}</th><th>${_t("Severity")}</th><th>${_t("Note")}</th></tr>
						</thead>
	...
	buildExcelXml() {
		...
		for (const group of data.kpiGroups || []) {
			...
			rows.push(
				buildRow([
					buildCell(_t("Metric"), "sHeader"),
					buildCell(_t("Value"), "sHeader"),
					buildCell(_t("Type"), "sHeader"),
					buildCell(_t("Description"), "sHeader"),
				])
			);
		...
		if ((data.topClients || []).length) {
			rows.push(buildRow([buildCell(_t("Top Clients Ranking"), "sSection", "String", 3)]));
			rows.push(
				buildRow([
					buildCell(_t("Rank"), "sHeader"),
					buildCell(_t("Client"), "sHeader"),
					buildCell(_t("Revenue"), "sHeader"),
					buildCell("", "sHeader"),
				])
			);
		...
		if (data.incomeVsExpenses) {
			rows.push(buildRow([buildCell(_t("Income vs Expenses"), "sSection", "String", 3)]));
			rows.push(
				buildRow([
					buildCell(_t("Income"), "sHeader"),
					buildCell(_t("Expenses"), "sHeader"),
					buildCell(_t("Balance"), "sHeader"),
					buildCell("", "sHeader"),
				])
			);
			rows.push(
		...
		if ((data.alerts || []).length) {
			rows.push(buildRow([buildCell(_t("Alerts"), "sSection", "String", 3)]));
			rows.push(
				buildRow([
					buildCell(_t("Alert"), "sHeader"),
					buildCell(_t("Value"), "sHeader"),
					buildCell(_t("Severity"), "sHeader"),
					buildCell(_t("Note"), "sHeader"),
				])
			);
